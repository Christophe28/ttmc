...
import { allCorrect, endMove, endBoard, leftToRigth, topToBot, rigthToLeft, backTopToBot, endGame } from "../../functions/canvas/conditions";
...
const Canvas = ({ players, setPlayers, data, setCurrentComponent }) => {
  ...
  const {score, x, y, xMax, yMax, yStart, xLast, yLast} = players[currentPlayer];
  ...
  useEffect(() => {
    if(!ctx) {return};
    createGameBoard(canvasRef.current, canvasWidth, canvasHeight, ctx, squareSize, typeQuestions, players, radius);
  }, [ctx]);
    ...
    if(active) {
      ...
      if(endBoard(x, xLast, y, yLast)) {
        ...
        if(endMove(move, squareSize, goodAnswer)){       
          ...
          restartDisplay(setGoodAnswer, setBetQuestion, setDisplayQuestions, setDisplayWrapper);
        }
        else {
          setComeBack(true);
          setActive(false);
      ...
      if(endMove(move, squareSize, goodAnswer)){  
        ...
        setPlayers((oldPlayers) => {
          ...
          newPlayers[currentPlayer].score = oldPlayers[currentPlayer].score + goodAnswer;
          return newPlayers;
        })
  ...
  useEffect(() => {
    if(score / 2 === 23) {
      setDataFilter(data.filter(category => category.main_theme.toLowerCase() === lastQuestion.type.toLowerCase()));
      setRandom(Math.floor(Math.random() * data.filter(category => category.main_theme.toLowerCase() === lastQuestion.type.toLowerCase()).length));
    }
    else {
      setDataFilter(data.filter(category => category.main_theme.toLowerCase() === typeQuestions[(score / 2) % 4].type.toLowerCase()));
      setRandom(Math.floor(Math.random() * data.filter(category => category.main_theme.toLowerCase() === typeQuestions[(score / 2) % 4].type.toLowerCase()).length));
    }
  }, [currentPlayer]);
